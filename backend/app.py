...
# In-memory user and event storage
users = []
events = []  # Mock database for events
...
def get_volunteer_history(username):
    for user in users:
        if user['username'] == username:
            return jsonify({'username': user['username'], 'history': user.get('history', [])}), 200
    return jsonify({'error': 'User not found'}), 404
...
def add_volunteer_history(username):
    for user in users:
        if user['username'] == username:
            ...
            user.setdefault('history', []).append(new_event)
